...
namespace IT_GAMK_Lab5.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //by default newly added users are added to role user
        ...
        //
        // POST : /Account/RemoveRoleFromUser
        ...
        // GET: /Account/ListUserRoles
        [Authorize(Roles = "Administrator")]
        public ActionResult ListUserRoles(string username)
        {
            ...
            UserViewModel model = new UserViewModel
            {
                UserId = user.Id,
                Email = user.Email,
                UserRoles = getUserRolesAsString(user),
                AvailableRolesForUser = getAvailableRolesForUserAsString(user)
            };
        ...
        //helper function
        //cuz i used the same code twice
        private List<string> getUserRolesAsString(ApplicationUser user)
        {
            List<string> roles = new List<string>();

            foreach (var role in user.Roles)
            {
                var matchingRole = RoleManager.Roles.Where(r => r.Id == role.RoleId).Select(r => r.Name).FirstOrDefault();
        ...
        private List<string> getAvailableRolesForUserAsString(ApplicationUser user)
        {
            List<string> availableRoles = new List<string>();
            foreach (var role in RoleManager.Roles)
            {
                var id = user.Roles.Where(r => r.RoleId == role.Id).Select(r => r.RoleId).FirstOrDefault();
